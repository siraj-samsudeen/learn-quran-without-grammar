...
**D11 — Verse entry schema (tightened)** (Q10, Q11, Q12). Dropped all derived fields (`word_count`, `juz`, `surah_name`, `cross_roots`). Replaced `fit_learn`/`fit_practice` with single `teaching_fit` field. Moved `arabic_fragment` inside `lesson_use` (it's lesson-specific, not verse-intrinsic). Dropped legacy cruft (`verified`, `type`, `pattern`, `previously_used_in_lesson_v1`, `audio_downloaded`, `reciter`, `audio_fragment` — audio metadata lives in `audio-plan.yaml`). Kept `roots[]` and `forms[]` as bounded arrays — conscious 1NF denormalization for read performance (picker's primary query is "filter verses by root").
